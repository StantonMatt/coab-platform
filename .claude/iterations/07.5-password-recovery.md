...
export async function solicitarReset(rut: string, ipAddress: string, logger: any) {
  ...
  try {
    ...
    if (!cliente || !cliente.telefono || !cliente.hash_contrasena) {
      ...
      return {
        success: true,
        message: 'Si el RUT existe y tiene teléfono registrado, recibirás un código por WhatsApp'
      };
    }
  ...
  const requestMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: 'Si tu RUT está registrado, recibirás un código por WhatsApp'
      });
      setStep('verify');
    },
    onError: (error: any) => {
      const message = error.response?.data?.error?.message || 'Error al solicitar código';
      toast({
        variant: 'destructive',
        title: 'Error',
        description: message
        ...
        <Card className="w-full max-w-md">
          ...
          <CardContent>
            <form onSubmit={resetForm.handleSubmit(onResetSubmit)} className="space-y-4">
              ...
              >
                {resetMutation.isPending ? 'Verificando...' : 'Cambiar Contraseña'}
              </Button>
...
1. Try non-existent RUT → Generic success (no enumeration)
2. Try 6 wrong codes → Token invalidated
3. Try 4 requests in 15 min → Rate limit error
